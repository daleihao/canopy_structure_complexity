...
%% import data
% POLDER
LAI_mean = importdata('trend_data/LAI_mean.csv', ',');
%LAI_std = importdata('trend_data/LAI_std.csv', ',');
EVI_mean = importdata('trend_data/EVI_mean.csv', ',');
...
LAI_mean(:,2) = LAI_mean(:,2)/10;
%LAI_std(:,2) = LAI_std(:,2)/10;
EVI_mean(:,2) = EVI_mean(:,2)/1e4;
...
text(2003, 2.4, ['Slope=' num2str(slope_value, '%.4f')], 'color', 'b', 'fontsize', 14)
text(2003, 2.2, ['P-value=' num2str(p_value, '%.4f')], 'color','b', 'fontsize', 14)

%text()
box on
axis([2003-0.5 2020.5 2 3.7])
ylabel('LAI')
xlabel('Year')
...
text(2003, 0.415, ['Slope=' num2str(slope_value, '%.4f')], 'color', 'b', 'fontsize', 14)
text(2003, 0.402, ['P-value=' num2str(p_value, '%.4f')], 'color','b', 'fontsize', 14)
%plot([0 0], [0 1], '--k', 'Linewidth', 1)


axis([2003-0.5 2020.5 0.39 0.5])
box on
xlabel('Year')
ylabel('EVI')
set(gca, 'Linewidth', 1.5, ...
