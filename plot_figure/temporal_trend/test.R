...
EVIs <- read_csv('all_points_EVI_pattern_mean.csv');
LAIs <- read_csv('all_points_LAI_pattern_mean.csv');
#EVIs$year <- as.factor(EVIs$year)
#LAIs$year <- as.factor(LAIs$year)

Alldata <- EVIs %>%
  left_join(LAIs, by = c('id','year')) %>%
  select(id, year, EVI, Lai) %>%
  mutate(EVI = EVI/1000, LAI = Lai/10) %>%
  select(id, year, EVI, LAI) %>%
  melt(id = c('id','year'))
...
ggsave("figure_4b_new_abs.tiff", plot = plot_figure, width = 15, height = 7, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")